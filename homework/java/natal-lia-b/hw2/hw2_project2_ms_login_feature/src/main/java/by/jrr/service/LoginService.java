...
public class LoginService {
    public boolean login(User user, String userInput) {
        if (!user.isBlocked()) {
            ...
            updateUsetStatus(user, result);
            return result;
        }
        return false;
    }

    private void updateUsetStatus(User user, boolean result) {
        if (result) {
            restoreAttempts(user);
        } else {
            reduceLoginAttempts(user);
            blockIfLoginAttemptsExceed(user);
        }
    }

    private void blockIfLoginAttemptsExceed(User user) {
